...
function greatestProduct(matrix) {
    ...
    for (let row = 0; row < matrix.length; row++) {
        for (let col = 0; col < matrix[row].length - 3; col++) {
            ...
            let verrticalProduct =
                if (matrix[row + 1] && matrix[row + 2] && matrix[row + 3]) {
                matrix[row][col] *
                matrix[row + 1][col] *
                matrix[row + 2][col] *
                        matrix[row + 3][col];
            ...
            if (verrticalProduct > biggestNumber) {
                biggestNumber = verrticalProduct;
            }
        }
    }
...
for (let i = 0; i < smallMatrix.length - 2; i++) {
    for (let j = 0; j < smallMatrix[i].length; j++) {
        const horizontalProduct = smallMatrix[i][j] * smallMatrix[i][j + 1];
        let verticalProduct;
        if (smallMatrix[i + 1]) {
            const verrticalProduct = smallMatrix[i][j] * smallMatrix[i + 1][j];
        }

        console.log(horizontalProduct, verrticalProduct);
    }
}
